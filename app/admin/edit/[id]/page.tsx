...
export default function EditProductPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 pb-24">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden">
        <div className="mb-8">
          <h1 className="text-2xl font-black text-slate-900">Edit Product</h1>
          <p className="text-slate-500 text-sm mt-1">Make changes and hit save.</p>
        </div>
        ...
        <form onSubmit={handleUpdate} className="space-y-8">
          ...
          <div className="pt-6 border-t border-gray-100">
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-3">Product Thumbnail</label>
            <div className="flex flex-col sm:flex-row items-center gap-6">
              <div className="w-40 h-40 rounded-2xl border border-gray-200 bg-slate-50 overflow-hidden shrink-0 flex items-center justify-center relative shadow-sm">
              ...
              <div className="flex-1 w-full border-2 border-dashed border-gray-200 rounded-2xl p-6 text-center hover:bg-gray-50 transition-colors flex flex-col justify-center items-center h-40">
                <p className="text-sm font-bold text-slate-600 mb-1">Replace Image</p>
                <p className="text-xs text-slate-400 mb-4">Select a new file to overwrite the current thumbnail.</p>
                <input 
                  type="file" 
                  accept="image/*" 
                  onChange={e => setNewImageFile(e.target.files?.[0] || null)} 
                  className="max-w-full text-sm text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" 
                />
              </div>
            </div>
          </div>
        ...
        <div className="mt-12 pt-8 border-t border-red-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-red-400 mt-1">Permanently delete this product from the database.</p>
          </div>
          <button
            type="button"
            onClick={handleDelete}
            disabled={loading}
            className="px-5 py-2.5 bg-red-50 text-red-600 hover:bg-red-600 hover:text-white font-bold rounded-xl transition-all disabled:opacity-50 flex items-center justify-center gap-2 border border-red-100 hover:border-red-600 w-full sm:w-auto"
          >
