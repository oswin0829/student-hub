...
export default function AddProductPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-8 pt-28 pb-24">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white dark:bg-gray-900 p-8 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-800">
        <div className="mb-8">
          <h1 className="text-2xl font-black text-slate-900 dark:text-white">Add New Product</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Create a new digital asset.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="bg-slate-50 dark:bg-gray-800/50 p-6 rounded-2xl border border-slate-100 dark:border-gray-700">
            <div className="flex items-center justify-between mb-6">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">Define IDs, labels, and pricing tiers.</p>
              </div>
            ...
            <div className="space-y-4">
              <AnimatePresence>
                {options.map((opt, index) => (
                  <motion.div key={index} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="relative flex flex-col sm:flex-row items-start gap-4 bg-white dark:bg-gray-900 p-5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm mt-2">
                    <div className="flex flex-col w-full sm:w-[25%]">
                      ...
                      <div className="flex items-center gap-2 bg-slate-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg p-2.5">
                        ...
                        <input type="text" placeholder="v1" value={opt.id} onChange={(e) => handleUpdateOption(index, 'id', e.target.value)} className="w-full text-sm outline-none bg-transparent font-mono text-slate-700 dark:text-white" />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col w-full sm:w-[45%]">
                      ...
                      <input type="text" placeholder="e.g. 1 Month" value={opt.label} onChange={(e) => handleUpdateOption(index, 'label', e.target.value)} className="w-full text-sm outline-none bg-slate-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded-lg p-2.5 font-semibold text-slate-700 dark:text-white" />
                    </div>
                    ...
                    <button type="button" onClick={() => handleRemoveOption(index)} className="absolute -top-3 -right-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-full p-1.5 shadow-sm transition-colors z-10">
                      <Trash2 size={14} />
                    </button>
                  </motion.div>
                ))}
              ...
              {options.length === 0 && (
                <div className="text-center py-6 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest border-2 border-dashed border-slate-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-900">
                  No Variants Added
                </div>
              )}
            </div>
          ...
          {/* --- THUMBNAIL UPLOAD --- */}
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Product Thumbnail</label>
            <div className="border-2 border-dashed border-gray-200 dark:border-gray-700 rounded-xl p-4 text-center hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              <input type="file" accept="image/*" required onChange={e => setImageFile(e.target.files?.[0] || null)} className="w-full text-sm text-slate-500 dark:text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-gray-100 dark:file:bg-gray-800 file:text-black dark:file:text-white hover:file:bg-gray-200 dark:hover:file:bg-gray-700" />
            </div>
          </div>
