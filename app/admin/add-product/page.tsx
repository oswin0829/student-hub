...
import { UploadCloud, Package, DollarSign, ArrowLeft, Tag, Plus, Trash2, Hash } from 'lucide-react';
...
export default function AddProductPage() {
  ...
  // Only keeping the public thumbnail image
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (options.some(opt => !opt.id || !opt.label || opt.price < 0)) return toast.error("Please ensure all options have an ID, a Label, and a valid Price.");
    ...
    try {
      ...
      const { error: dbError } = await supabase.from('products').insert([{
        name, price: parseFloat(price), category, options, image_url: imageUrl
      }]);
  ...
  return (
    <div className="max-w-3xl mx-auto p-8 pb-24">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-8 rounded-[2rem] shadow-sm border border-gray-100">
        <div className="mb-8"><h1 className="text-2xl font-black text-slate-900">Add New Product</h1><p className="text-slate-500 text-sm mt-1">Create a new digital asset.</p></div>

        <form onSubmit={handleSubmit} className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2"><Package size={14} /> Product Name</label><input type="text" required value={name} onChange={e => setName(e.target.value)} className="w-full p-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-600 outline-none" /></div>
            <div className="space-y-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2"><Tag size={14} /> Category</label><input type="text" required value={category} onChange={e => setCategory(e.target.value)} className="w-full p-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-600 outline-none" /></div>
            <div className="space-y-2 md:col-span-2"><label className="text-xs font-bold text-slate-500 uppercase tracking-wider flex items-center gap-2"><DollarSign size={14} /> Base Price (RM)</label><input type="number" step="0.01" required value={price} onChange={e => setPrice(e.target.value)} className="w-full p-3 rounded-xl border border-gray-200 focus:ring-2 focus:ring-blue-600 outline-none" /></div>
          </div>

          {/* --- THE UPGRADED VARIANT BUILDER --- */}
          <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100">
            <div className="flex items-center justify-between mb-6">
              <div>
                <label className="text-sm font-bold text-slate-700 block">Product Options (Variants)</label>
                <p className="text-xs text-slate-500">Define the ID, Label, and Price.</p>
              </div>
